        ...
        <transition name="slide">
            <div class="details-box" v-show="showDetail">
                ...
                <ul class="details-content">
                    <li v-for="(item,index) in selectFoods"  :key="item.name" class="jus-between">
...
export default {
  ...
  computed: {
    ...
    totalPrice() {
      let total = 0;
      this.selectFoods.forEach((item, index, arr) => {
  ...
  methods: {
    itemPrice(index) {
      return this.selectFoods[index].price * this.selectFoods[index].count;
    },
    onShowDetail() {
